...
export class UploadDialogComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        if (this.entityType !== 'program') {
            this.getLearningUnitOptions().subscribe({
                next: (options) =>
                    this.existingLearningUnitOptions.set(options),
                error: (err) => this.handleApiError(err),
            });
        }
        ...
        group.controls.existingUnitId.valueChanges.subscribe((selectedId) => {
            if (selectedId) {
                const selectedUnit = this.originalLearningUnits.find(
                ...
                if (selectedUnit) {
                    const basePayload = {
                        ...selectedUnit,
                        ...(this.selectedFile() && {
                            finalTaskFile: this.selectedFile(),
                        }),
                    ...
                    if (this.entityType === 'section') {
                        ...
                        this.close.emit({
                            createdTopic: {
                                ...topicFromPayload,
                                learningResources:
                                    topicFromPayload.learningResources ?? [],
    ...
    private getLearningResourceCount(): number {
        return Object.keys(this.form.controls).filter((key) =>
            key.startsWith('learningResource')
        ).length;
    }
    ...
    private handleApiError(err: any) {
        ...
        if (errorObj && typeof errorObj === 'object') {
            ...
            this.errorText.set(
                this.translate.instant('dialog.error.unknownEror')
            );
        }
    }
